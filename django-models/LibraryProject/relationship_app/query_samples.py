...
def retrieve_librarian_for_library(library_name):
    ...
    try:
        ...
        # Method 1: Query the librarian for this library using the reverse OneToOne relationship
        librarian = library.librarian

        # Method 2: Alternative approach using Librarian.objects.get(library=)
...
def retrieve_librarian_for_library_alternative(library_name):
    """
    Alternative method to retrieve the librarian for a library.
    Demonstrates OneToOne relationship using Librarian.objects.get(library=).
    """
    try:
        # Get the library object
        library = Library.objects.get(name=library_name)

        # Query the librarian for this library using Librarian.objects.get(library=)
        librarian = Librarian.objects.get(library=library)
